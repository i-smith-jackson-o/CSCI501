...
int main() {
  ...
  // Calculate each produces sales
  calcSales(units, prices, sales); 

  // Sort the elemnts in the sales array in descending 
  // order and shuffle the id numbers in the id array 
...
void initVectors(vector<int> &id, 
		 vector<int> &units,
		 vector<double> &prices) {
